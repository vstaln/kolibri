...
function selftest() {
  ...
  const buildPage = fs.readFileSync(path.join(ROOT, 'build-page.cjs'), 'utf8');
  assert.ok(
  ...
  execFileSync(process.execPath, [path.join(ROOT, 'build-page.cjs'), '--check'], { cwd: ROOT, stdio: 'pipe' });
  for (const file of [
    'course-app.mjs',
    'course-content.mjs',
